...
generateAPARef <- function(df) {
  ...
  # loop through each row of the dataframe
    ...
    entry <- paste0(entry,".")
    if (abstract != "") { 
      entry <- paste(entry, " <button style='position: absolute;left: -35px; top: 0;line-height: 1.2;border-width: 0px;' type='button' data-bs-toggle='collapse' data-bs-target='#ref-",number,"'   aria-expanded='false'  aria-controls='ref-",number,"' class='btn btn-sm btn-danger'>+</button>", sep = "")
      kws <-""
      for (kk in 1:length(keywords)) {
        kws <- paste0(kws,"<span style='margin-right: 2px;' class='badge rounded-pill bg-warning'>",keywords[kk],"</span>")
      }
