...
npubs = nrow(Publications) - nrow(Publications |> filter(`Document Type` == "Thesis"))
...
for (p in 1:length(pub_types)) {
  ...
  theh2 <- ifelse(pub_types[p] == "Thesis", "Theses", paste0(pub_types[p],"s"))
  pub_tags[[p]] <- htmltools::tagList(
    
      htmltools::tags$h2(theh2),
        year_tags
    )
}
(pub_tags)
